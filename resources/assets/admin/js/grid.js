...
const getElemClassAll = (className) => [
  ...document.querySelectorAll(className),
];
const fadeOutEffect = (elem, time = 200) => {
  elem.style.opacity = window
...
const fadeToggleEffect = (elem, time = 200, display = "inline-block") => {
  ...
  currentDisplay === "none"
    ? fadeInEffect(elem, time, display)
    : fadeOutEffect(elem, time);
};
...
const fillWidthElement = (elem, width = 100, timeout = 20) => {
  var elementWidth =
    (elem.getBoundingClientRect().width / getWindowWidth()) * 100;
  ...
  if (elementWidth !== width && currentValue !== width)
    var effect = setInterval(() => {
      ...
      if (elementWidth < width || currentValue < width) {
        if (currentValue >= width) clearInterval(effect);
      } else {
        if (currentValue <= width) clearInterval(effect);
      }
    }, timeout);
